...
from sqlalchemy import DateTime, ForeignKey, String, Text, UniqueConstraint, Boolean
...
class User(Base):
    ...
    role: Mapped[str] = mapped_column(String(20), nullable=False)  # MANAGER, INSPECTOR, CONTRACTOR
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    email: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    phone: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    company: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    trades: Mapped[Optional[list[str]]] = mapped_column(ARRAY(Text), nullable=True)
