...
async def _enrich_project(project: Project, db: AsyncSession) -> dict:
    """Add computed counts to a project."""
    building_count = await db.scalar(
        select(func.count()).where(Building.project_id == project.id)
    )
    ...
    data["total_buildings"] = building_count or 0
...
async def list_projects(
    ...
    return [ProjectResponse(**await _enrich_project(p, db)) for p in projects]
